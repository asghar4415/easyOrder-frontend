...
import { 
  ...
  Users, ListFilter, MapPin, Edit3, ExternalLink 
} from "lucide-react";

export default function RestaurantDetailPage() {
  const { id } = useParams();
  const router = useRouter();
  const { fetchCompleteRestaurantData } = useSuperAdminRestaurants();
    ...
    <div className="max-w-6xl mx-auto space-y-5 pb-10">
      ...
      <div className="flex items-center justify-between">
        <button 
          ...
          className="flex items-center gap-2 text-gray-500 hover:text-orange-500 transition-colors text-xs font-semibold"
        >
      ...
      <div className="relative h-44 rounded-2xl overflow-hidden bg-gray-100 border border-gray-200 dark:border-gray-800">
        ...
        <div className="absolute bottom-4 left-6 flex items-end gap-4">
          ...
          <div className="w-16 h-16 rounded-xl bg-white p-1 shadow-lg border border-gray-100">
             <img src={data.logo} className="w-full h-full object-cover rounded-lg" alt="logo" />
          </div>
          <div className="pb-1">
            <h1 className="text-xl font-bold text-white leading-tight">{data.name}</h1>
            <p className="text-gray-200 text-xs flex items-center gap-1.5 opacity-90">
        ...
        <div className="lg:col-span-3 space-y-5">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden shadow-sm">
            ...
            <div className="flex px-6 border-b border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-white/5">
              {["info", "staff", "menu"].map(tab => (
                <button 
                  ...
                  className={`py-3.5 px-4 text-[11px] font-bold uppercase tracking-wider transition-all relative ${
            ...
            <div className="p-6">
               ...
               {activeTab === "staff" && renderStaffTab(data)}
               {activeTab === "menu" && renderMenuTab(data)}
            </div>
          </div>
        </div>

        {/* Right Column: Actions (1/4 width) */}
        <div className="space-y-5">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 shadow-sm">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">Management</h3>
            <div className="space-y-2.5">
              <button className="w-full flex items-center justify-center gap-2 bg-orange-500 text-white py-2.5 rounded-lg text-sm font-bold hover:bg-orange-600 transition-all">
                <Edit3 size={16} /> Edit Details
              </button>
              <button className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-bold border border-gray-200 dark:border-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-50 transition-all">
          ...
          <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-5 text-white shadow-md">
             ...
             <p className="text-[10px] mt-1.5 leading-relaxed opacity-90">Fee applied to all processed transactions.</p>
          </div>
        </div>
      </div>
    </div>
...
function renderInfoTab(data: any) {
  return (
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="p-3.5 bg-orange-50/50 dark:bg-orange-500/5 rounded-xl border border-orange-100 dark:border-orange-900/30">
             ...
             <p className="text-sm font-bold text-gray-800 dark:text-white">{data.admin.email}</p>
          </div>
        </div>
        <div className="space-y-2.5">
           ...
           <div className="flex items-center gap-3 text-xs text-gray-600 dark:text-gray-400 p-2">
             <Phone size={14} className="text-gray-400"/> {data.phone}
           </div>
           <div className="flex items-center gap-3 text-xs text-gray-600 dark:text-gray-400 p-2 border-t border-gray-50 dark:border-white/5">
             <Mail size={14} className="text-gray-400"/> {data.email}
           </div>
        </div>
      </div>
    </div>
...
function renderStaffTab(data: any) {
  return (
    <div className="space-y-3">
      {data.staff.map((s: any) => (
        <div key={s.id} className="flex items-center justify-between p-3.5 border border-gray-100 dark:border-gray-800 rounded-xl hover:bg-gray-50 transition-colors">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center font-bold text-sm text-gray-500">
          ...
          <button className="text-[10px] font-bold text-gray-400 hover:text-red-500 uppercase tracking-tighter">Revoke Access</button>
        </div>
      ))}
    </div>
  );
...
function renderMenuTab(data: any) {
  return (
      ...
      {data.categories.map((cat: any) => (
        <div key={cat.id} className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-bold text-gray-400 uppercase">{cat.items.length} Items</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {cat.items.map((item: any) => (
              <div key={item.id} className="flex gap-3 p-3 border border-gray-100 dark:border-gray-800 rounded-xl bg-gray-50/20">
                {item.image && <img src={item.image} className="w-14 h-14 rounded-lg object-cover border border-gray-100" alt={item.name} />}
